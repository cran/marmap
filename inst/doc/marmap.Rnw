...
plot(papoue, image=T, bpal=blues(100))
@
...
plot(papoue, image=T, bpal=blues(100),
        ...
        col=c("lightgrey","darkgrey","black"), lwd=c(0.8,0.8,1), lty=c(1,1,1), 
        drawlabel=c(F,F,F))
@
...
<<fig=FALSE,echo=TRUE>>=
get.transect(papoue, 151,-6, 153, -7, distance=T)
@
...
<<fig=TRUE,echo=TRUE>>=
get.transect(papoue, 151,-6, 153, -7, distance=T) -> transect
...
write.table(sampling, "sampling.csv", sep=",", quote=F, row.names=F)
@
...
head(samp) # a little preview of the first 6 of the 16 lines of the dataset. 
plot(papoue, image=T, bpal=blues(100),
        deep=c(-9000,-3000,0), shallow=c(-3000,-10, 0), step=c(1000, 1000, 0), 
        col=c("lightgrey","darkgrey","black"), lwd=c(0.8,0.8,1), lty=c(1,1,1), 
        drawlabel=c(F,F,F))
# add points from the sampling.csv, and add text to the plot:
points(samp$x, samp$y, pch=21, col="black", bg="yellow", cex=1.3)
...
plot(papoue, deep=c(-4500,0), shallow=c(-50,0), step=c(500,0), 
     lwd=c(0.3,1), lty=c(1,1), col=c("grey","black"), drawlabels=c(F,F))	
...
points(sp[,1:2], col="black", bg=blues[-sp$depth], pch=21,cex=1.5)
require(shape)
...
The function \verb"get.area()" returns a surface area un square kilometers (\verb"$Square.Km"), and a matrix of zeros and ones delimiting the area of interest. The \verb"$Lon", \verb"$Lat" and \verb"$Area" objects can be used to display these areas: 
...
legend(x="bottomleft",legend=c(paste("bathyal:",ba,"km2"), paste("abyssal:",ab,"km2")), 
		col="black", pch=21, pt.bg=c(rgb(.7,0,0,.3), rgb(.7,.7,0,.3)) )
@
...
get.box(atl,
              x1=-68.6, x2=-53.7, y1=42.4, y2=32.5, 
              width=3, col="red") -> out
@
<<fig=TRUE,echo=TRUE>>=
library(lattice)             
...
\subsection{Getting bathymetric data from NOAA: read.bathy()}
...
<<fig=FALSE,echo=TRUE>>=
subsetSQL(min_lon=145,max_lon=150, min_lat=-2, max_lat=0) -> test
summary(test)
@
